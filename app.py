...
from scoring_engine import (
    calc_composite_score, kelly_position_size,
    ...
    def add_signal(self, sig: dict):
        with self._lock:
            ...
            self.active_signals = sorted(
                self.active_signals, key=lambda x: x.get("score", 0), reverse=True
            )[:20]
...
def update_all_data():
    from data_fetcher import (
        ...
        fetch_earnings_calendar, fetch_us_futures, fetch_sector_etf,
    ...
    try:
        ...
        if not q["valid"]:
            logger.warning(f"宏觀數據品質問題: {q['issues']}")
    ...
    try:
        ...
        logger.warning(f"session: {e}")

    try:
        state.fred_data = fetch_fred_data()
    except Exception as e:
        logger.debug(f"fred: {e}")

    try:
        state.earnings_calendar = fetch_earnings_calendar()
    except Exception as e:
        logger.warning(f"earnings: {e}")

    try:
        state.us_futures_data = fetch_us_futures()
    except Exception as e:
        logger.warning(f"futures: {e}")

    try:
        state.sector_etf_data = fetch_sector_etf()
    except Exception as e:
        logger.warning(f"sector: {e}")

    try:
        state.macro_data["crypto_sentiment"] = fetch_crypto_sentiment()
    except Exception as e:
        logger.warning(f"crypto: {e}")
...
def _scan_symbol(symbol: str):
    ...
    try:
        ...
        if not risk.get("can_signal"):
            reason = (
                risk.get("blockers", ["未知"])[0][:50]
                if risk.get("blockers") else "風控阻止"
            )
    ...
    try:
        ...
        if not state.trump_data:
            state.trump_data = {
                ...
                "main_event_type": "none", "error": str(e),
            }
...
def morning_briefing():
    ...
    try:
        ...
        state.daily_briefing = {
            "ai_available": False,
            "overall_environment": (
                "適合交易" if env_score >= 70 else
                "今日觀望" if env_score < 40 else "謹慎操作"
            ),
            "environment_reason": f"VIX {vix:.1f}，恐懼貪婪 {fg:.0f}，{regime_zh}",
            "best_opportunities": (
                ["BTCUSD（加密 24/7）", "ETHUSD"] if is_weekend else
                ["XAUUSD（黃金）", "GBPUSD", "EURUSD"] if vix < 25 else []
            ),
...
def api_state():
    try:
        try:
            ...
            logger.debug(f"market_session: {e}")

        snap = state.get_snapshot()
        snap["active_signals"] = [
            s for s in snap.get("active_signals", [])
            if s.get("category") not in STOCK_CATS
            and s.get("symbol") not in STOCK_SYMBOLS
        ]
...
def api_performance():
    ...
    try:
        ...
        logger.error(f"get_real_performance: {e}")
        real = {
            "total": 0, "wins": 0, "losses": 0,
            "win_rate": 0, "total_pnl": 0,
            "data_source": "error",
            "note": f"績效計算錯誤：{str(e)[:100]}",
        }
...
def api_watchdog_status():
    ...
    queue_sz = (
        len(getattr(__import__("watchdog"), "rate_limiter", object())._queue)
    ...
    for sym in syms:
        try:
            ...
            if d and d.get("closes"):
                prices[sym] = d["closes"][-20:]
        except Exception:
            pass
    sl  = list(prices.keys())
    mat = {
        s1: {
            s2: (1.0 if s1 == s2 else calc_rolling_correlation(prices[s1], prices[s2], 20))
            for s2 in sl
        }
        for s1 in sl
    }
