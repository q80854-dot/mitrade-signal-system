...
app.py v5.2 — 完整最終版
所有修復：
  [嚴重] composite_score NameError → 掃描全部崩潰
  Bug 1: AI 調整後低分仍進訊號列表
  Bug 2: expire_old() DB result 未更新
  Bug 3: 冷啟動保護 + market_session 快取過期修正
  Bug 4: trump_data 失敗無回退值
  Bug 5: daily_briefing AI 失敗無降級邏輯
  Bug 6: 週末不掃加密貨幣
  Bug 7: expire_old() 後 signal_log 未重新載入
  Bug 8: market_session 殘留舊日期（weekday 停留昨天）
  Bug 9: watchdog 例外後 status=dead
  移除美股品種掃描，Render 付費版無需 keep-alive
"""
...
class SystemState:
    ...
    def get_snapshot(self):
        with self._lock:
            ...
            # ★ Bug 8：確保 Sharpe 不是 Inf/NaN
            if perf and "sharpe" in perf:
                if not math.isfinite(perf["sharpe"]):
                    perf["sharpe"] = 0
            return {
...
def run_scan():
    ...
    try:
        ...
        if is_weekend:
            ...
            syms_to_scan = sorted(
                [s for s in SYMBOLS if not SYMBOLS[s].get("monitor_only")
                 and SYMBOLS[s].get("cat") not in STOCK_CATS and s not in STOCK_SYMBOLS],
                key=lambda s: SYMBOLS[s].get("priority", 3))
        ...
        watchdog.record_failure(str(e))
        # ★ Bug 9：例外後也 ping，避免 watchdog status=dead
        try: watchdog.ping()
        except: pass
...
def api_state():
    try:
        # ★ Bug 8 修復：每次呼叫都更新 market_session（無快取，幾乎零成本）
        # 防止 state.market_session 殘留昨天的 weekday
        try:
...
def api_kelly():
    ...
    if not wr or not rr:
        stats = store.get_trade_stats()
        wr = wr or stats.get("win_rate",50)/100; rr = rr or 1.6
    else: wr /= 100
