"""
app.py — 主程式
雲端版：Flask + APScheduler + Telegram
24小時運行於 Render
"""
import logging
import threading
...
class SystemState:
    def __init__(self):
        ...
        self.signal_log:     List[Dict] = []  # 所有歷史訊號記錄（勝率追蹤用）
    ...
    def add_signal(self, sig: Dict):
        with self._lock:
            ...
            self.signal_log.append({
                ...
                "result":    "pending",  # pending / tp1 / tp2 / sl / expired
            })
            # 只保留最新 200 筆記錄
...
def run_scan():
    ...
    if env_score < 20:
        msg = format_alert_message(
            "🚨 市場極度危險",
            f"VIX={state.system_status.get('vix','?')} 極端恐慌，今日所有訊號暫停",
            "danger"
        )
        send_message(msg)
        state.scan_count += 1
    ...
    for symbol in symbols_to_scan:
        try:
            ...
            if signal:
                ...
                send_message(msg)
                logger.info(f"✅ Signal: {symbol} {signal['direction']} {signal['score']}/100")
    ...
    state.scan_count += 1
    state.last_scan_time = datetime.now(timezone.utc).strftime("%Y-%m-%d %H:%M UTC")
    logger.info(f"Scan done. New signals: {len(new_signals)}")
    ...
    try:
        ...
        for post in trump.get("posts", []):
            if post.get("impact_level") == "high":
                msg = format_alert_message(
                    "⚡ 川普重大發文",
                    f"<b>原文：</b>{post.get('original_text','')}\n\n"
                    f"<b>AI解讀：</b>{post.get('ai_interpretation','')}\n"
                    f"<b>影響品種：</b>{', '.join(post.get('affected_assets',[]))}\n\n"
...
def start_scheduler():
    ...
    # 每 15 分鐘掃描
    scheduler.add_job(run_scan,         "interval", minutes=SYSTEM["scan_interval_min"], id="scan")
    # 每 30 分鐘查川普
    scheduler.add_job(check_trump,      "interval", minutes=SYSTEM["trump_check_min"],   id="trump")
    # 每天 00:30 UTC 產生簡報
    scheduler.add_job(morning_briefing, "cron",     hour=0, minute=30,                  id="briefing")
    # 每 5 秒輪詢指令
    scheduler.add_job(poll_commands,    "interval", seconds=5,                           id="commands")
