...
def _scan_symbol(symbol):
    try:
        ...
        if not risk.get("can_signal"): return None
...
def start_scheduler():
    try:
        ...
        s.add_job(run_scan,         "interval", minutes=SYSTEM["scan_interval_min"], id="scan",     misfire_grace_time=60)
        s.add_job(check_trump,      "interval", minutes=SYSTEM["trump_check_min"],   id="trump",    misfire_grace_time=60)
        s.add_job(morning_briefing, "cron",     hour=0, minute=30,                   id="briefing", misfire_grace_time=300)
        s.add_job(poll_commands,    "interval", seconds=5,                            id="commands", misfire_grace_time=10)
        s.start()
