...
class SystemState:
    ...
    def add_signal(self, sig):
        with self._lock:
            ...
            sig["fill_price"]=slip["fill_price"]; sig["slippage_pct"]=slip["slippage_pct"]
            stats=store.get_trade_stats(); wr=stats.get("win_rate",50)/100; rr=sig.get("rr1",1.5)
            kelly=kelly_position_size(wr,rr,CB.get("risk_per_trade_pct",2)/100)
            sig["kelly_risk_pct"]=kelly["kelly_pct"]; sig["kelly_edge"]=kelly.get("edge",0)
            self.active_signals=[s for s in self.active_signals if not(s.get("symbol")==symbol and s.get("direction")==direction)]
            self.active_signals.append(sig)
            self.active_signals=sorted(self.active_signals,key=lambda x:x.get("score",0),reverse=True)[:20]
    ...
    def calc_win_rate(self):
        ...
        return {"total":stats.get("total",0),"wins":stats.get("tp",0),"losses":stats.get("sl",0),"win_rate":stats.get("win_rate",0)}

    def get_snapshot(self):
        with self._lock:
            wr=self.calc_win_rate()
            equity=store.get_equity_curve(); trades=store.get_trade_list()
            perf=calc_performance_metrics(equity,trades) if len(equity)>=2 else {}
            return {
                "active_signals":list(self.active_signals),"macro_data":dict(self.macro_data),
                "trump_data":dict(self.trump_data),"daily_briefing":dict(self.daily_briefing),
                "system_status":dict(self.system_status),"market_session":dict(self.market_session),
                "fred_data":dict(self.fred_data),"earnings_calendar":list(self.earnings_calendar),
                "us_futures_data":dict(self.us_futures_data),"sector_etf_data":dict(self.sector_etf_data),
                "signal_log":list(self.signal_log[-100:]),"win_rate":wr,"performance":perf,
                "regime":dict(self.regime),"last_scan_time":self.last_scan_time,
                "scan_count":self.scan_count,"signal_count":self.signal_count,
                "version":self.version,"disclaimer":DISCLAIMER,
            }
...
def update_all_data():
    from data_fetcher import (fetch_macro_data,fetch_market_status,fetch_fred_data,
                               fetch_earnings_calendar,fetch_us_futures,fetch_sector_etf,fetch_crypto_sentiment)
...
def _scan_symbol(symbol):
    ...
    if consec>=3: logger.info(f"  [{symbol}] ⏸️ 防連虧暫停（已連虧{consec}次）"); return None
    regime=state.regime; allowed=regime.get("allowed_directions",["buy","sell"])
    if not allowed: logger.info(f"  [{symbol}] 市場機制停止交易"); return None
    try:
        ...
        for tf_key,tf_data in tf.items():
            ...
            if q["valid"] and q["data"].get("data_quality",{}).get("cleaned"): tf[tf_key]=q["data"]
        ...
        if not risk.get("can_signal"):
            ...
            logger.info(f"  [{symbol}] Step3 ⛔ {reason}"); return None
        ...
        auto=auto_composite_score(macro_data=state.macro_data,indicators=indicators,
                                   trump_data=state.trump_data,symbol=symbol,
                                   category=SYMBOLS.get(symbol,{}).get("cat",""))
        record_weight_history(auto); composite_score=auto["composite_100"]
        logger.info(f"  [{symbol}] 自適應評分={composite_score:.1f} 主導={auto['dominant_state']}")
        if composite_score<THRESH["min_score"]: return None

        sig=generate_signal(symbol,tf,state.macro_data)
        if not sig: return None
        sig["score"]=composite_score; sig["dominant_state"]=auto["dominant_state"]

        corr=check_portfolio_correlation(symbol,direction,state.active_signals)
        if corr["correlated"]:
            sig["risk_warnings"]=[corr["message"]]; sig["score"]=max(0,sig["score"]+corr["score_penalty"])
        else: sig["risk_warnings"]=risk.get("warnings",[])
        ...
        logger.error(f"  [{symbol}] 掃描異常: {e}"); watchdog.record_failure(str(e)); return None

def run_scan():
    logger.info(f"{'='*40}\n=== 掃描 #{state.scan_count+1} 開始 ===")
    try:
        update_all_data(); state.expire_old()
        env=state.system_status.get("env_score",100); sess=state.market_session.get("session_zh","—")
        ...
        for symbol in syms_to_scan:
            try:
                ...
                if sig:
                    state.add_signal(sig); new_signals.append(sig)
        ...
        if state.scan_count%3==0:
            ...
            safe_send(format_scan_summary(scan_num=state.scan_count,new_signals=new_signals,
                active_count=len(state.active_signals),scores=state._last_scores,
                session=sess,vix=vix,regime_zh=regime_zh,win_rate=wr.get("win_rate",0)),priority=8)
...
def _quick_macro_update():
    try:
        ...
        macro=fetch_macro_data()
        if macro: macro["_initialized"]=True; state.macro_data=macro
        state.market_session=fetch_market_status()
    except Exception as e: logger.warning(f"Quick macro: {e}")

@app.route("/api/signals")
def api_signals(): return jsonify({"signals":state.active_signals,"count":len(state.active_signals)})
...
def api_macro():
    ...
    return jsonify({"macro":state.macro_data,"session":state.market_session,"status":state.system_status,
                    "fred":state.fred_data,"futures":state.us_futures_data,"sector":state.sector_etf_data,
...
def api_performance():
    ...
    stats=store.get_trade_stats()
    wr=stats.get("win_rate",50)/100; kelly=kelly_position_size(wr,1.6,3000)
    return jsonify({"performance":perf,"stats":stats,"kelly":kelly,"equity_curve":equity[-100:],"n_trades":len(trades)})
...
def api_backtest_symbol(symbol):
    symbol=symbol.upper()
    if symbol not in SYMBOLS: return jsonify({"error":f"品種 {symbol} 不存在"}),400
...
def api_backtest_all():
    def _run():
        from backtester import run_full_backtest; run_full_backtest()
...
def api_scoring(symbol):
    ...
    try:
        ...
        mtf=check_multi_timeframe(tf); ind=calc_all_indicators(tf.get("entry",{}))
        sc=calc_composite_score(ind,mtf.get("direction","buy"),mtf.get("bull_timeframes",[]),mtf.get("bear_timeframes",[]))
...
def api_earnings(): return jsonify({"earnings":state.earnings_calendar})

@app.route("/api/quotes")
def api_quotes():
    quotes={}; macro=state.macro_data
...
def api_signal_result(sig_id):
    ...
    with state._lock:
        for s in state.active_signals:
            if s.get("id")==sig_id:
                state.active_signals.remove(s); state.expired_signals.append(s); break
        for s in state.signal_log:
            if s.get("id")==sig_id:
                symbol=s.get("symbol","")
                if result=="sl": store.set_consec_loss(symbol,store.get_consec_loss(symbol)+1)
                elif result in ["tp1","tp2"]: store.set_consec_loss(symbol,0)
                break
...
def api_signal_close(sig_id):
    data=request.get_json() or {}; close_price=float(data.get("close_price",0))
    target=None
    with state._lock:
        for s in state.active_signals:
            if s.get("id")==sig_id: target=s; break
    if not target: return jsonify({"error":"訊號不存在或已到期"}),404
    direction=target.get("direction","buy"); entry=target.get("entry_price",close_price)
    ...
    with state._lock: state.active_signals=[s for s in state.active_signals if s.get("id")!=sig_id]
    state.signal_log=store.load_signal_log(500)
    return jsonify({"ok":True,"id":sig_id,"pnl_usd":pnl_usd,"result":result_type})

@app.route("/api/trump")
def api_trump(): return jsonify(state.trump_data or {"has_impact_posts":False,"posts":[]})

@app.route("/api/briefing")
def api_briefing(): return jsonify(state.daily_briefing or {"ai_available":False})
...
def api_correlation():
    ...
    syms=["EURUSD","GBPUSD","USDJPY","XAUUSD","BTCUSD","US500","NAS100"]; prices={}
    for sym in syms:
        try:
...
def api_equity():
    from config import ACCOUNT_BALANCE_USD
    curve=store.get_equity_curve(); trades=store.get_trade_list()
...
def api_scan_force():
    ...
    if _t.time()-last<60: return jsonify({"ok":False,"error":"60秒內只能觸發一次"}),429
...
def api_news(symbol):
    ...
    except Exception as e: return jsonify({"symbol":symbol.upper(),"news":[],"error":str(e)}),500
