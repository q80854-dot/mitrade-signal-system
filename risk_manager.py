"""
risk_manager.py v5.3 — 修正版
修正：
  ★ get_system_status 移除美股類別建議
  ★ 超短線：新聞暫停從 30 分鐘改為 15 分鐘
  ★ 每日最大損失上限從 6% 改為 4%（超短線更嚴格）
"""
import logging
from datetime import datetime, timezone, timedelta
from typing import Dict, List, Optional
from config import (CIRCUIT_BREAKER as CB, SYMBOLS, ACCOUNT_BALANCE_USD,
                    MIN_ACCOUNT_FOR_INDEX, MAX_SIMULTANEOUS_TRADES, CORRELATION_GROUPS)

logger = logging.getLogger(__name__)

EIA = {"weekday": 2, "hour": 14, "minute": 30}  # 週三 14:30 UTC
...
def check_vix_circuit_breaker(macro_data):
    ...
    v = vix.get("price", 0)
    if v >= CB["vix_extreme"]:
        return {"triggered": True, "level": "extreme", "value": v,
                "message": f"🚨 VIX {v:.1f} 極度恐慌！所有訊號暫停", "action": "stop_all"}
    elif v >= CB["vix_threshold"]:
        return {"triggered": True, "level": "high", "value": v,
...
def check_eia_schedule():
    ...
    ws  = et - timedelta(minutes=CB["eia_pause_minutes"])
    we  = et + timedelta(minutes=CB["eia_pause_minutes"])
    if ws <= now <= we:
        return {"near_eia": True, "message": "⚠️ EIA 原油報告時間，石油訊號暫停"}
            ...
            try:
                ...
                if -1 <= days_to <= CB["earnings_pause_days"]:
                    return {"near_earnings": True, "days_to": days_to,
                            "message": f"⚠️ {si.get('name',symbol)} 財報在 {days_to} 天後",
                            "score_penalty": -15}
            except: pass
...
def check_daily_loss_limit():
    today     = datetime.now(timezone.utc).strftime("%Y-%m-%d")
    # ★ 超短線：每日最大損失 4%（原本 6%）
    max_daily = ACCOUNT_BALANCE_USD * 0.04
    if _daily_loss["date"] != today:
            ...
            if sig.get("symbol") in group and sig.get("symbol") != new_symbol:
                if sig.get("direction", "") == new_direction:
                    return {"correlated": True, "with_symbol": sig["symbol"],
                            "message": f"⚠️ {new_symbol} 與現有 {sig['symbol']} 高度相關",
                            "score_penalty": -10}
...
def check_weekend_gap() -> dict:
    ...
    if now.weekday() == 4 and now.hour >= 20:
        return {"warning": True, "message": "⚠️ 週五收盤前，建議縮減或平倉"}
    if now.weekday() == 0 and now.hour < 2:
        return {"warning": True, "message": "⚠️ 週一開盤，留意週末跳空"}
...
def check_margin_usage(active_signals: List[dict]) -> dict:
    ...
    if total_risk_pct > 10:
        return {"warning": True, "total_risk_pct": round(total_risk_pct, 1),
                ...
                "message": f"⚠️ 總風險敞口 {total_risk_pct:.1f}%，建議控制在 6% 以下"}
...
def check_price_spike(symbol, tf_data):
    ...
    if rng >= CB["price_spike_pct"]:
        return {"spike": True, "range": round(rng, 2),
                "message": f"⚠️ {symbol} 近期波動 {rng:.1f}%，等待穩定"}
    return {"spike": False}
...
def run_all_checks(symbol, tf_data, macro_data, active_signals=None):
    ...
    if cat == "指數" and checks["vix"].get("value", 0) > 25:
        warnings.append("⚠️ VIX偏高，指數風險加倍"); score_adj -= 10
...
def get_system_status(macro_data):
    ...
    # ★ 移除美股類別建議
    cat_advice = {}
    for cat in ["外匯", "商品", "指數", "加密"]:
        min_acc = MIN_ACCOUNT_FOR_INDEX if cat == "指數" else 0
        if ACCOUNT_BALANCE_USD < min_acc:
            cat_advice[cat] = "⚠️ 帳戶不足"
        elif score >= 70:
            cat_advice[cat] = "✅ 適合交易"
        elif score >= 40:
            cat_advice[cat] = "⚠️ 謹慎"
    ...
    return {
        "env_score":      max(0, score),
        "env_status":     st,
        "env_color":      cl,
        "vix":            vix,
        "vix_status":     vix_c.get("level", "normal"),
        "fg_score":       fg.get("score", 50),
        "fg_label":       fg.get("label_zh", "中性"),
        "can_trade":      score >= 50,
