...
from config import (CIRCUIT_BREAKER as CB, SYMBOLS, ACCOUNT_BALANCE_USD,
                    ...
                    MAX_SIMULTANEOUS_TRADES, CORRELATION_GROUPS)
...
def check_account_requirement(symbol):
    si=SYMBOLS.get(symbol,{}); min_acc=si.get("min_account",0)
    ...
    if ACCOUNT_BALANCE_USD<min_acc:
        return {"sufficient":False,"required":min_acc,"current":ACCOUNT_BALANCE_USD,
            "message":f"⚠️ {si.get('cat','')}建議帳戶 ${min_acc}，目前 ${ACCOUNT_BALANCE_USD:.0f}","warning_only":True}
...
def record_signal_loss(loss_usd: float):
    ...
    if _daily_loss["date"]!=today:
        _daily_loss["date"]=today; _daily_loss["loss_usd"]=0.0; _daily_loss["signal_count"]=0
    _daily_loss["loss_usd"]+=loss_usd; _daily_loss["signal_count"]+=1

def check_daily_loss_limit():
    today=datetime.now(timezone.utc).strftime("%Y-%m-%d")
    max_daily=ACCOUNT_BALANCE_USD*0.06
    if _daily_loss["date"]!=today:
    ...
    return {"exceeded":False,"today_loss":round(loss,2),"max_loss":round(max_daily,2),"remaining":round(max_daily-loss,2)}
...
def check_max_positions(active_signals: List[dict]) -> dict:
    ...
    return {"exceeded":False,"count":count,"max":MAX_SIMULTANEOUS_TRADES,"remaining":MAX_SIMULTANEOUS_TRADES-count}
    ...
    for group in CORRELATION_GROUPS:
        ...
        for sig in active_signals:
            if sig.get("symbol") in group and sig.get("symbol")!=new_symbol:
                if sig.get("direction","")==new_direction:
                    return {"correlated":True,"with_symbol":sig["symbol"],
                        "message":f"⚠️ {new_symbol} 與現有 {sig['symbol']} 高度相關，雙重持倉增加風險","score_penalty":-10}
    return {"correlated":False}

def check_weekend_gap() -> dict:
    now=datetime.now(timezone.utc)
    if now.weekday()==4 and now.hour>=20:
        return {"warning":True,"message":"⚠️ 週五收盤前，建議縮減或平倉，避免週末跳空"}
    if now.weekday()==0 and now.hour<2:
        return {"warning":True,"message":"⚠️ 週一開盤，留意週末期間是否有重大新聞"}
...
def check_margin_usage(active_signals: List[dict]) -> dict:
    ...
    if total_risk_pct>15:
        return {"warning":True,"total_risk_pct":round(total_risk_pct,1),"total_risk_usd":round(total_risk_usd,2),
            "message":f"⚠️ 總風險敞口 {total_risk_pct:.1f}%，建議控制在 10% 以下"}
    return {"warning":False,"total_risk_pct":round(total_risk_pct,1),"total_risk_usd":round(total_risk_usd,2)}
...
def run_all_checks(symbol, tf_data, macro_data, active_signals=None):
    ...
    si=SYMBOLS.get(symbol,{}); cat=si.get("cat","")
    checks={
    ...
    return {"status":status,"warnings":warnings,"blockers":blockers,"checks":checks,"score_adj":score_adj,"can_signal":len(blockers)==0}

def get_system_status(macro_data):
    vix_c=check_vix_circuit_breaker(macro_data); fg=macro_data.get("fear_greed",{})
    vix=vix_c.get("value",0); score=100
    ...
    return {"env_score":max(0,score),"env_status":st,"env_color":cl,"vix":vix,
            "vix_status":vix_c.get("level","normal"),"fg_score":fg.get("score",50),
            "fg_label":fg.get("label_zh","中性"),"can_trade":score>=50,
            "category_advice":cat_advice,"daily_loss":daily}
