"""
adaptive_weight_engine.py v5.2
全自動七維度權重調整引擎
"""
import logging
...
CATEGORY_BASE = {
    ...
    "美股": {"tech":28,"macro":15,"trump":15,"fg":8, "news":18,"vol":9,"trend":7},
}
...
def calc_adaptive_weights(macro_data: dict, indicators: dict = None,
    ...
    if category == "美股":
        ...
        if [e for e in (earnings or []) if e.get("symbol")==symbol]:
            deltas["news"] += 15; deltas["tech"] -= 10; deltas["trend"] -= 5
            adjustments.append({"dim":"news","delta":15,
                "reason":f"{symbol} 財報日，新聞情緒主導"})
...
def calc_dim_scores(macro_data: dict, indicators: dict,
                     trump_data: dict, symbol: str = "") -> dict:
    from config import SYMBOLS
    vix     = float(macro_data.get("vix",{}).get("price",20) or 20)
    fg      = float(macro_data.get("fear_greed",{}).get("score",50) or 50)
    sp500_c = float(macro_data.get("sp500",{}).get("chg",0) or 0)
    ...
    fed_rate = float((fred.get("fed_rate",{}) or {}).get("value",5) or 5)
...
def auto_composite_score(macro_data: dict, indicators: dict,
                          trump_data: dict, symbol: str = "",
                          category: str = "") -> dict:
    from config import SYMBOLS
    if not category and symbol:
        category = SYMBOLS.get(symbol,{}).get("cat","")
    ...
    if dim_scores["tech"]>0.6 and dim_scores["macro"]>0.6:
        composite = min(1.0, composite+0.05)
    if dim_scores["trump"]>0.6 and dim_scores["tech"]>0.6:
...
def record_weight_history(result: dict):
    ...
    if len(_weight_history) > 100:
        _weight_history.pop(0)
