...
# ── 各類別策略參數（根據10年回測優化）──
# ── v5.0 策略參數（根據10年高強度回測優化）──
# 改進：精確ADX分級、縮短TP讓更多訊號到達、外匯止損縮小、防連虧
CATEGORY_PARAMS = {
    "外匯": {
        "min_score":      68,    # v5：重新校準分數系統後調整
        "min_adx":        25,    # v5：ADX≥25（比v4的30更合理）
        "sl_mult":        0.8,   # 縮小止損，提高勝率
        "tp1_mult":       1.5,   # v5：縮短TP1，提高到達率
        "tp2_mult":       2.5,   # v5：縮短TP2
        "max_hold_days":  7,
        "require_weekly_trend": True,
        "require_dxy_confirm":  True,
    },
    "商品_黃金": {
        "min_score":      62,    # v5：稍微放寬，增加訊號
        "min_adx":        20,
        "sl_mult":        1.3,   # v5：縮小止損
        "tp1_mult":       1.8,   # v5：縮短TP1
        "tp2_mult":       3.0,
        "max_hold_days":  12,
        "require_inflation_confirm": True,
    },
    "商品_WTI": {
        "min_score":      72,    # v5：維持高門檻
        "min_adx":        28,
        "sl_mult":        1.2,
        "tp1_mult":       1.8,
        "tp2_mult":       3.0,
    ...
    "加密": {
        "min_score":      58,    # v5：降低門檻（高波動，分數天花板低）
        "min_adx":        18,
        "sl_mult":        1.8,   # v5：高波動但縮小止損
        "tp1_mult":       2.2,   # v5：縮短TP1
        "tp2_mult":       4.0,
        "max_hold_days":  20,
        "require_bull_market":   True,
        "fg_min_score":   20,    # v5：更嚴格的F&G過濾
    },
    "指數": {
        "min_score":      65,
        "min_adx":        20,
...
def _check_category_filters(symbol: str, direction: str,
    ...
    if cat == "外匯":
        ...
        if adx < cp.get("min_adx", 30):
            return False, f"外匯ADX{adx:.0f}<{cp.get('min_adx',30)}，趨勢不足"
        # DXY 方向確認（用宏觀數據的dxy）
        dxy_chg = macro_data.get("dxy", {}).get("chg", 0)
        if symbol in ["EURUSD","GBPUSD","AUDUSD"]:
            # 這些品種與DXY反向
            if direction=="buy" and dxy_chg > 0.3:
                return False, f"DXY上升{dxy_chg:.1f}%，外匯做多風險高"
            if direction=="sell" and dxy_chg < -0.3:
                return False, f"DXY下降{dxy_chg:.1f}%，外匯做空風險高"
        return True, ""
