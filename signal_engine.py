""" signal_engine.py v5.2 — 修正版
修正：
  1. TIER_D_SYMBOLS 移除 HK50/AUDUSD/USDCAD（這些是正常品種）
  2. 加密貨幣使用品種專屬門檻 55 分而非全域 65 分
"""
import logging
...
# ★ Bug 修正：只封鎖真正不穩定的品種，移除 HK50/AUDUSD/USDCAD
TIER_D_SYMBOLS = {"TSLA"}
...
def calc_trading_costs(symbol, entry):
    ...
    return {
        "spread":round(spread_pips*pip,6),"spread_pips":spread_pips,
...
def check_multi_timeframe(tf_data):
    ...
    return {
        "direction":direction,"score":score,"resonance":resonance,
...
def generate_signal(symbol, tf_data, macro_data):
    try:
        ...
        # ★ Bug 修正：使用品種專屬門檻（加密 55，外匯 65，等）
        cat_min_score = cp.get("min_score", THRESH["min_score"])
        if score < cat_min_score: return None
        ...
        if not pass_filter:
            logger.info(f"  [{symbol}] 類別過濾：{reason}")
            return None
        ...
        return {
            "id":sig_id,"symbol":symbol,"name":si.get("name",symbol),"emoji":si.get("emoji","📊"),
