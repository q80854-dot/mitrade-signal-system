...
def calc_ema_system(closes):
    ...
    e9,e21,e50,e200=(ema(closes,P["ema_fast"]),ema(closes,P["ema_mid"]),
                     ema(closes,P["ema_slow"]),ema(closes,P["ema_trend"]))
    ...
    e9p=ema(closes[:-1],P["ema_fast"]); e21p=ema(closes[:-1],P["ema_mid"])
    if e9p and e21p:
...
def calc_rsi_divergence(closes, lookback=20):
    ...
    pl=min(recent[-10:]); ph=max(recent[-10:]); rl=min(rsi_vals[-10:]); rh=max(rsi_vals[-10:])
    ppl=min(recent[:10]); pph=max(recent[:10]); prl=min(rsi_vals[:10]); prh=max(rsi_vals[:10])
...
def calc_macd(closes):
    ...
    ef=ema_series(closes,P["macd_fast"]); es=ema_series(closes,P["macd_slow"])
    ...
    prev_sig=ema(ml[:-1],P["macd_signal"]); prev_hist=ml[-2]-prev_sig if prev_sig and len(ml)>=2 else 0
...
def calc_adx(highs,lows,closes,period=None):
    ...
    if n<period*2+5: return {"valid":False,"reason":"數據不足","value":0,"trend":"無趨勢","strong":False}
    ...
    if len(dx_list)<period: return {"valid":False,"reason":"DX不足","value":0,"trend":"無趨勢","strong":False}
...
def calc_fibonacci(highs,lows,closes,lookback=50):
    ...
    return {"valid":True,"swing_high":round(sh,4),"swing_low":round(sl,4),"uptrend":uptrend,
            "levels":levels,"nearest_above":min(above,default=None),"nearest_below":max(below,default=None),
...
def calc_all_indicators(data):
    ...
    return {"valid":True,"ema":ema_s,"rsi":rsi,"macd":macd,"atr":atr,"bb":bb,
            ...
            "candlestick":cp,"candlestick_pattern":cp,"rsi_divergence":rd,"adx":adx,
