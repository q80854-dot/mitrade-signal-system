...
def fetch_macro_data():
    ...
    # ── 方法2：Finnhub 備援（yfinance 失敗時補充）──
    missing = [k for k in ["sp500","gold","btc"] if k not in macro]
    if missing and FINNHUB_API_KEY:
        ...
        for key in missing:
            fsym = finnhub_map.get(key)
            if not fsym: continue
            try:
                import time as _t
                now_ts = int(_t.time())
                ...
                if r.status_code == 200:
                    d = r.json()
                    p = d.get("c",0); pv = d.get("pc",p)
                    if p and p > 0:
                        macro[key] = {"price":round(p,4),"chg":round((p-pv)/pv*100,2) if pv else 0,"prev":round(pv,4)}
                _t.sleep(0.2)
            except Exception as e:
                logger.debug(f"Macro finnhub {key}: {e}")

    # ── 方法3：CoinGecko 補充 BTC ──
    if "btc" not in macro:
        try:
            ...
            if r.status_code == 200:
                ...
                if p:
                    macro["btc"] = {"price":round(p,0),"chg":round(chg,2),"prev":round(p/(1+chg/100),0) if chg else p}
        except Exception as e:
            logger.debug(f"Macro coingecko btc: {e}")

    # ── 方法4：Alternative.me 補充 VIX 替代指標 ──
    if "vix" not in macro:
        try:
            ...
            if r.status_code == 200:
                d = r.json()
                if d.get("data"):
                    fg = float(d["data"][0]["value"])
                    # 用恐懼貪婪反推 VIX 估算（僅作顯示用）
                    vix_est = round(12 + (100-fg)*0.3, 1)
                    macro["vix"] = {"price":vix_est,"chg":0,"prev":vix_est,"estimated":True}
        except: pass
    # CNN 恐懼貪婪指數（多個備用來源）
    fg_score = None
...
def fetch_fred_data():
    ...
    # 靜態備用數據（FRED 超時時顯示最近已知值）
    FRED_FALLBACK = {
        "fed_rate":      {"value":5.25,"prev":5.50,"chg":-0.25,"date":"2024-09","note":"備用"},
    ...
    for key, fallback in FRED_FALLBACK.items():
        if key not in fred:
            fred[key] = fallback

    if "yield_curve" not in fred:
        fred["yield_curve"] = {"spread":0.15,"inverted":False,"label_zh":"正常","note":"備用"}
