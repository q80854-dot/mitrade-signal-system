""" data_fetcher.py v5.2 — 修正版
修正：
  1. fetch_market_status 加入 crypto_tradeable=True 欄位
  2. _fred_fallback 移除寫死舊數據，改為顯示「—」避免誤導
  3. 移除美股相關 fetch 邏輯
"""
...
def _cross_validate_price(prices: dict, symbol: str) -> dict:
    ...
    return {
        "price":round(weighted,6),"sources":list(trusted.keys()),"rejected":rejected,
...
def _yf_bars(yahoo_sym, period, interval, bars):
    ...
    try:
        ...
        logger.warning(f"yfinance {yahoo_sym}: {e}"); return None
...
def fetch_macro_data():
        ...
        for key, sym in {"vix":"^VIX","dxy":"DX-Y.NYB","sp500":"^GSPC"}.items():
            try:
                ...
                if h is not None and len(h) >= 2:
                    ...
                    if valid:
                        macro[key] = {"price":round(p,4),"chg":round((p-pv)/pv*100,3),"prev":round(pv,4),"source":"yfinance"}
                time.sleep(0.2)
    ...
    if FINNHUB_API_KEY:
        try:
            ...
            if r.status_code==200 and r.json().get("c"):
                gold_prices["finnhub"] = float(r.json()["c"])
        except: pass
    if gold_prices:
        ...
        if cv["price"] > 0:
            ...
            macro["gold"] = {"price":cv["price"],"chg":chg,"prev":round(prev,2),
                             "sources":cv["sources"],"confidence":cv["confidence"],"source":"+".join(cv["sources"])}
    ...
    if btc_prices:
        ...
        if cv["price"] > 0:
            ...
            macro["btc"] = {"price":round(cv["price"],0),"chg":round(chg,2),"prev":round(prev,0),
                            "sources":cv["sources"],"confidence":cv["confidence"],
                            "spread_pct":cv.get("spread_pct",0),"source":"+".join(cv["sources"])}
    ...
    macro["data_quality"] = {"filled_sources":filled,"total_sources":6,
...
def _fred_fallback():
    # ★ 修正：移除寫死的舊數據，改為空值避免誤導七維度評分
    return {
        "fed_rate":    {"value":None,"prev":None,"chg":None,"date":"—","note":"API未設定"},
        "cpi":         {"value":None,"prev":None,"chg":None,"date":"—","note":"API未設定"},
        "core_cpi":    {"value":None,"prev":None,"chg":None,"date":"—","note":"API未設定"},
        "unemployment":{"value":None,"prev":None,"chg":None,"date":"—","note":"API未設定"},
        "yield_curve": {"spread":None,"inverted":False,"label_zh":"無數據","note":"API未設定"},
    }

def fetch_fred_data():
    if cc := _cache_get("fred_data", 3600): return cc
    if not FRED_API_KEY:
        return _cache_set("fred_data", _fred_fallback())
    fred = {}
    for key, sid in {"cpi":"CPIAUCSL","core_cpi":"CPILFESL","gdp":"GDP",
                     "unemployment":"UNRATE","fed_rate":"FEDFUNDS",
                     "us10y_yield":"GS10","us2y_yield":"GS2"}.items():
        try:
            d = _req("https://api.stlouisfed.org/fred/series/observations",
                     params={"series_id":sid,"api_key":FRED_API_KEY,"file_type":"json","limit":2,"sort_order":"desc"},
                     timeout=5)
            if d and "observations" in d and d["observations"]:
                ...
                try:
                    ...
                    fred[key] = {"value":round(v,3),"prev":round(pv,3),"chg":round(v-pv,3),
                                 "date":obs[0]["date"],"source":"fred"}
                except (ValueError, KeyError): pass
            time.sleep(0.2)
        except Exception as e:
            logger.debug(f"FRED {key}: {e}"); continue
...
def fetch_earnings_calendar(days_ahead=7):
    ...
    for item in d["earningsCalendar"]:
        ...
        if sym in watched:
            results.append({"symbol":sym,"name":SYMBOLS.get(sym,{}).get("name",sym),
...
def fetch_company_news(symbol, days_back=2):
    ...
    return _cache_set(ck, [{"headline":i.get("headline",""),"summary":i.get("summary","")[:200],
...
def fetch_market_status():
    ...
    if is_weekend:
        s, sz, t = "weekend", "週末休市", False
    elif 22<=h or h<8:
        s, sz, t = "asia",     "亞洲盤",         True
    elif 8<=h<13:
        s, sz, t = "london",   "倫敦盤",          True
    elif 13<=h<17:
        s, sz, t = "overlap",  "倫紐重疊（最佳）", True
    else:
        s, sz, t = "newyork",  "紐約盤",          True
    ...
    return {
        "session":        s,
        "session_zh":     sz,
        "tradeable":      t,               # 外匯/指數/商品是否可交易
        "crypto_tradeable": True,          # ★ 修正：加密貨幣永遠可交易
        "hour_utc":       h,
        "weekday":        wd,
        "best_symbols":   best.get(s,[]),
