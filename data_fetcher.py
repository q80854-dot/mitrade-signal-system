...
def _av_bars(av_sym, interval):
    ...
    for _, ohlcv in list(data[ts_key].items())[:120]:
        try:
            out.append({"ts":0,"open":round(float(ohlcv["1. open"]),6),
                "high":round(float(ohlcv["2. high"]),6),"low":round(float(ohlcv["3. low"]),6),
                "close":round(float(ohlcv["4. close"]),6),"volume":int(float(ohlcv.get("5. volume",0)))})
        except: continue
...
def fetch_macro_data():
    ...
    if YFINANCE_OK:
        for key, sym in {"vix":"^VIX","dxy":"DX-Y.NYB","us10y":"^TNX","sp500":"^GSPC","gold":"GC=F","btc":"BTC-USD"}.items():
            try:
    ...
    try:
        d = _req("https://production.dataviz.cnn.io/index/fearandgreed/graphdata",timeout=8)
        if d:
            fg = d.get("fear_and_greed",{})
            s  = float(fg.get("score",50))
            macro["fear_greed"] = {"score":round(s,1),"label":fg.get("rating","neutral"),
                "label_zh":"極度恐慌" if s<20 else "恐慌" if s<40 else "中性" if s<60 else "貪婪" if s<80 else "極度貪婪"}
    except: macro["fear_greed"] = {"score":50,"label":"neutral","label_zh":"中性"}
    return _cache_set("macro_data", macro)

# ── FRED 美聯儲數據 ──

def fetch_fred_data():
    if c := _cache_get("fred_data", 3600): return c
    if not FRED_API_KEY: return {}
    series = {"cpi":"CPIAUCSL","core_cpi":"CPILFESL","gdp":"GDP",
    ...
    for key, sid in series.items():
        ...
        if d and "observations" in d and d["observations"]:
            obs = d["observations"]
            try:
                v,pv = float(obs[0]["value"]),float(obs[1]["value"]) if len(obs)>1 else float(obs[0]["value"])
                fred[key] = {"value":round(v,3),"prev":round(pv,3),"chg":round(v-pv,3),"date":obs[0]["date"]}
            except: pass
        time.sleep(0.2)
...
def fetch_earnings_calendar(days_ahead=7):
    ...
    now,end = datetime.now(timezone.utc), datetime.now(timezone.utc)+timedelta(days=days_ahead)
...
def fetch_economic_calendar():
    ...
    if not FINNHUB_API_KEY: return []
    now,end = datetime.now(timezone.utc), datetime.now(timezone.utc)+timedelta(days=7)
...
def fetch_us_futures():
    ...
    if futures:
        ...
        futures["overall"] = {"bias":"bullish" if avg>0.1 else "bearish" if avg<-0.1 else "neutral",
            "avg_chg":round(avg,2),
...
def fetch_crypto_sentiment():
    ...
    try:
        ...
        if d and "data" in d:
            ...
            result = {"market_cap_change_24h":chg,"btc_dominance":btc,
