"""
ai_analyst.py v4.1 — Grok AI 分析模組
修正：移除已棄用的 live_search（HTTP 410）
Grok 3 本身已有網路知識，不需要額外工具參數
"""
...
def _call_grok(prompt: str, max_tokens: int = 800) -> Optional[str]:
    """呼叫 Grok API（不需要 tools 參數，Grok 3 內建知識）"""
    if not GROK_API_KEY:
...
def monitor_trump_posts() -> Dict:
    """
    使用 Grok 的即時知識監控川普最新發文
    Grok 3 有 X/Truth Social 的即時知識，不需要 live_search 工具
    """
